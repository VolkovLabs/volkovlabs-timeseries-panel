...
export const TooltipPlugin2 = ({
  ...
  useLayoutEffect(() => {
    ...
    let isWasClick = false;
    let isFeezeNow = false;
    ...
    /**
     * fires on mouse leaves freezed box
     */
    const downEventLeaveMouse = (e: Event) => {
      if (isFeezeNow) {
        dismiss();
      }
    };
    ...
    const _render = () => {
      ...
      if (pendingPinned) {
        ...
        if (_isPinned) {
          ...
          domRef.current?.addEventListener('mouseleave', downEventLeaveMouse);
        } else {
    ...
    const dismiss = () => {
      if (isFeezeNow) {
        /**
         * Avoid to blink between pin action (set cursor position)
         * and reset freeze box
         */
        setTimeout(() => {
          if (domRef.current != null) {
            domRef.current.style.padding = '0px';
          }
        }, 100);
      }
    ...
    config.addHook('setCursor', (u) => {
      ...
      const initialTop = u.rect.top + top;
      const initialLeft = u.rect.left + left;
    ...
    /**
     * Use with debounce to set freeze box if cursor doesn`t move
     * fires on mousemoves
     */
    config.addHook(
      'setCursor',
      debounce((u) => {
        /**
         * Do not freeze the field if the mouse leaves the series field, the cursor stops when you click on a point, or the cursor does not contain a series.
         */
        if (!_isHovering || isWasClick || !closestSeriesIdx) {
          return;
        }

        let { left = -10, top = -10 } = u.cursor;

        const plotCursorTop = u.rect.top + top;
        const plotCursorLeft = u.rect.left + left;

        /**
         * Compare last plot cursor position with current cursor position
         */
        if (currentCursorPosition.top === plotCursorTop && currentCursorPosition.left === plotCursorLeft) {
          if (left >= 0 || top >= 0) {
            let clientX = u.rect.left + left;
            let clientY = u.rect.top + top;
          ...
          /**
           * Repeat behavior from Click action
           */
          setTimeout(() => {
            _isPinned = true;
            isFeezeNow = true;
